...
export default function Home() {
      ...
      const notesData = await Promise.all(
        snapshot.docs.map(async (doc) => {
          ...
          return {
            ...
            title: `${data.subject || "Untitled"} - Sem ${data.semester || "N/A"}`
          };
        })
      );
  ...
  const calculateStats = (notes) => {
    ...
    const branches = [...new Set(notes.map(note => note.branch))].filter(Boolean);
  ...
  const categories = ["All", ...new Set(notes.map(note => note.category || "Basic"))];
  const semesters = ["All", ...new Set(notes.map(note => note.semester || "N/A"))];
  const branches = ["All", ...new Set(notes.map(note => note.branch || "N/A"))];
  ...
  const StatCard = ({ icon: Icon, title, value, color }) => (
    <div className={`bg-gradient-to-br ${color} p-5 rounded-xl shadow-lg text-white`}>
      ...
      <div className="min-h-screen bg-gradient-to-b from-gray-50 to-gray-100">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          <div className="bg-gradient-to-r from-blue-600 to-indigo-700 rounded-2xl p-8 md:p-12 mb-8 text-white overflow-hidden relative">
            ...
            <div className="relative z-10 max-w-3xl">
              ...
              <p className="text-lg text-blue-100 mb-8">
                Discover, share, and excel with curated study materials from Computer Science students
              </p>
          ...
          <div className="mb-8 flex flex-col sm:flex-row justify-between gap-4">
            <div className="flex flex-wrap gap-2">
              ...
              <div className="bg-white px-3 py-2 rounded-lg border border-gray-200 shadow-sm">
                ...
                >
                  {branches.map(br => (
                    <option key={br} value={br}>Branch: {br}</option>
                  ))}
                </select>
              </div>
            </div>
            ...
            <div className="fixed inset-0 z-50 overflow-y-auto p-4 bg-gray-500 bg-opacity-75">
              <div className="bg-white rounded-lg shadow-xl max-w-xs mx-auto p-4">
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    <h4 className="text-sm font-medium text-gray-900 mb-2">Semester</h4>
                  ...
                  <div>
                    <h4 className="text-sm font-medium text-gray-900 mb-2">Branch</h4>
                    <div className="grid grid-cols-2 gap-2">
                      {branches.map((branch) => (
                        <button
                          key={branch}
          ...
          <section className="bg-gradient-to-br from-indigo-50 to-blue-50 rounded-2xl p-8 mb-12 border border-indigo-100">
            <h2 className="text-2xl font-bold text-gray-800 mb-8 text-center">Why Our Platform</h2>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              <div className="bg-white rounded-xl p-6 shadow-sm hover:shadow-md transition-shadow duration-300 border border-gray-100">
                ...
                <h3 className="text-xl font-semibold text-gray-800 mb-3">Diverse Content</h3>
                <p className="text-gray-600">
                  Comprehensive collection covering all branches, courses, and subjects for complete academic coverage.
                </p>
              </div>
              ...
              <div className="bg-white rounded-xl p-6 shadow-sm hover:shadow-md transition-shadow duration-300 border border-gray-100">
                ...
                <h3 className="text-xl font-semibold text-gray-800 mb-3">Smart Filtering</h3>
                <p className="text-gray-600">
                  Find exactly what you need with our advanced filtering by category, semester, branch, and subject.
                </p>
              </div>
              ...
              <div className="bg-white rounded-xl p-6 shadow-sm hover:shadow-md transition-shadow duration-300 border border-gray-100">
                ...
                <p className="text-gray-600">
                  Community-rated content with detailed metadata ensures you get the best study materials.
                </p>
              </div>
            </div>
          </section>
